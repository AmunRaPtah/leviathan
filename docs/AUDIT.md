...
| # | Item | Status | Notes |
|---|------|--------|-------|
| 1 | Retrieval/query API | ✅ | `leviathan rag` (JSON CLI) **and** `leviathan serve` — a stdlib HTTP API (`/retrieve`, `/health`, `/facts`, `/discover`) with bearer auth, for remote consumers (the Pardalos bot). |
...
| — | Sentence/section-boundary-aware + configurable chunking | ✅ | `documents._chunk_sentences`; `LEVIATHAN_CHUNK_WORDS/OVERLAP/SENTENCE_AWARE` |
...
| — | Validation phase (text length, word-count, id format) | ✅ | `validate.py`; `leviathan validate`; runs in `harvest` |
| — | Structured JSON logging / observability | ✅ | `obs.py` (`LEVIATHAN_LOG_JSON`/`LEVIATHAN_LOG_FILE`); wired into `net` + `harvest` |
