...
Thresholds are read from the environment on every call (LEVIATHAN_Q_*), so a deploy —
...
# Production defaults. Overridden per-call by LEVIATHAN_Q_<NAME> if set.
_DEFAULTS = {
    "MIN_TITLE_CHARS": 8,
    "MIN_BODY_WORDS": 40,
...
def _t(name: str) -> float:
    """Current threshold for `name` — env override (LEVIATHAN_Q_<name>) or default."""
    raw = os.environ.get(f"LEVIATHAN_Q_{name}")
    if raw is None:
