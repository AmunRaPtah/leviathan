...
human-readable `print` output the pipeline already produces:
    ...
    LEVIATHAN_LOG_JSON=1            -> one JSON object per line on stderr
    LEVIATHAN_LOG_FILE=/path/log    -> append JSON lines to a file (implies JSON)
...
def _enabled() -> bool:
    return bool(os.environ.get("LEVIATHAN_LOG_JSON") or os.environ.get("LEVIATHAN_LOG_FILE"))


def _sink():
    path = os.environ.get("LEVIATHAN_LOG_FILE")
    if path:
