...
DOMAIN = "social science, geopolitics, power, and human behaviour — politics, war, propaganda, intelligence, organized crime, cults, and business strategy"
...
SYSTEM = (
    ...
    "base covering social science, geopolitics, power, and human behaviour — politics, war, propaganda, intelligence, organized crime, cults, and business strategy, plus a few evidence excerpts.\n\n"
    "HARD RULES:\n"
...
def _agentic(sheet: str, snippets: str, topic: str | None, max_turns: int = 12) -> str:
    ...
    task = (
        f"You are analysing the Leviathan knowledge base. Pre-computed facts are in "
...
def _compose(topic: str | None, narrative: str, sheet: str, cites: list[tuple]) -> str:
    ...
    title = f"Leviathan report — {topic}" if topic else "Leviathan landscape report"
...
def generate(topic: str | None = None, *, agent: bool = False, con=None,
    ...
    try:
        ...
        emailed = None
        if email:
            subject = f"Leviathan report — {topic}" if topic else "Leviathan landscape report"
            try:
