"""HTTP retrieval API — exposes Leviathan's RAG to remote consumers (e.g. the Pardalos bot).
...
Auth: if LEVIATHAN_API_KEY is set, requests must send `Authorization: Bearer <key>`
...
def serve(host: str = "127.0.0.1", port: int = 8800) -> None:
    ...
    auth = "on" if os.environ.get("LEVIATHAN_API_KEY") else "OFF (set LEVIATHAN_API_KEY)"
    print(f"[serve]   Leviathan API on http://{host}:{port}  (auth: {auth})")
