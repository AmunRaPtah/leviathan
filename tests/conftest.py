"""Shared fixtures: redirect Leviathan's data paths to a temp dir, offline only."""

from __future__ import annotations

import pytest

from leviathan import config, storage
...
def env(tmp_path, monkeypatch):
    ...
    for name in ("MIN_TITLE_CHARS", "MIN_BODY_WORDS", "MIN_ABSTRACT_WORDS"):
        monkeypatch.setenv(f"LEVIATHAN_Q_{name}", "0")
    return data
