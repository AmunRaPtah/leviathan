...
# Local secrets (RESEND_API_KEY, LEVIATHAN_EMAIL_TO, PATENTSVIEW_API_KEY) — untracked.
...
# outrun the watchdog. Set LEVIATHAN_EMBED_BACKEND=st for higher-quality manual runs.
export LEVIATHAN_EMBED_BACKEND="${LEVIATHAN_EMBED_BACKEND:-lsa}"
...
systemd-run --scope --quiet --collect -p MemoryMax=4G -p MemorySwapMax=2G \
  ...
  "$PROJECT/.venv/bin/python" -m leviathan harvest --topics "$TOPICS" --limit "$LIMIT" >>"$LOG" 2>&1
rc=$?
